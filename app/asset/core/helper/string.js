...
export function className(string) {
    return _.chain(string).split(/[\.\/\\\\// ]/g).map(
        (value) => _.chain(value).camelCase().upperFirst().value()
    ).value().join('.')
}
...
export function toBoolean(string) {
    return /(true|1)/g.test(String(string).toLowerCase()) ? true : false
}
...
export function httpBuildQuery(formdata, numeric_prefix, arg_separator) {
    var value, key, tmp = [],
        that = this
